...
impl<'a> Cursor<'a> {
    ...
    fn next(&mut self) -> TokenType {
        ...
        match char {
            ...
            _ => {
                eprintln!(
                    "Unknown char: Fix the error msg later: {char}: {}",
                    char as u8
                );
                todo!()
            }
    ...
    fn string(&mut self) -> TokenType {
        ...
        if self.eof() {
            let err = RxError::Lex(Error::new(
                token!(
                    String,
                    self.content(),
                    Span::new(self.start, self.pos(), self.line)
                ),
                "Unterminated string",
            ));
            eprintln!("{}", err);
        ...
        TokenType::String(content.to_string().into_boxed_str())
    }
    ...
    fn number(&mut self) -> TokenType {
        ...
        TokenType::Number(self.content().to_string().into_boxed_str())
    }
    ...
    fn ident(&mut self) -> TokenType {
        ...
        match symbol {
            ...
            _ => TokenType::Ident(symbol.to_string().into_boxed_str()),
        }
    }
}
